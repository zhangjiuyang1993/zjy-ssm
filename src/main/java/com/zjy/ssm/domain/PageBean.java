...
public class PageBean<T> {
    //已知数据
    private int pageNum;//当前页，从页面那边传过来
    private int pageSize;//每页显示的数据条数
    private int totalRecord;//总的记录条数。查询数据库得到的数据

    //需要计算得来
    private int totalPage;//总页数，通过totalRecord和pageSize计算可以得来
    //开始索引，也就是我们从数据库中要从第几行数据开始拿，有了startIndex和pageSize
    //就知道了limit语句中的两个数据，就能获得每页需要显示的数据了
    private int startIndex;

    //将每页要显示的数据放在list集合中
    private List<T> list;

    //分页要显示的页数，比如在上面上显示1,2,3,4,5页，start就为1，end为5，这个也是算出来的
    private int start;
    private int end;
    ...
    public PageBean(int pageNum, int pageSize, int totalRecord) {
        ...
        if (totalPage <= 5) {
            ...
            //总页数大于5，就要根据当前是第几页，来判断start和end为多少
